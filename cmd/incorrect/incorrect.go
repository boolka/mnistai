...
func main() {
	var netFilename, cfgFilename, outputDir string
	...
	flag.StringVar(&netFilename, "filename", "ai.net", "ai network filename to load from")
	flag.StringVar(&netFilename, "f", "ai.net", "ai network filename to load from")
	flag.StringVar(&outputDir, "out", "", "incorrect images output folder")
	flag.StringVar(&outputDir, "o", "", "incorrect images output folder")
	...
	for {
		...
		if int(label) != predictedLabel {
			totalIncorrect++

			f, err := os.Create(filepath.Join(outputDir, fmt.Sprintf("%d_label_%d_predicted_%d.png", i, label, predictedLabel)))
			if err != nil {
				log.Fatal(err)
			}

			img := buf.GetNRGBAImg(testIdx.ImagesHeader)
	...
	println("total incorrect:", totalIncorrect)
}
